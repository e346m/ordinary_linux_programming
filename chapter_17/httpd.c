...
static void trap_signal(int sig, sighandler_t handle);
static void signal_exit(int sig);
static void service(FILE *in, FILE *out, char *docroot);
static void free_request(struct HTTPRequest *req);
static struct HTTPRequest* read_request(FILE *in);
static void read_request_line(struct HTTPRequest *req, FILE *in);
static struct HTTPHeaderField* read_request_field(FILE *in);
...
static void do_file_response(struct HTTPRequest *req, FILE *out, char *docroot) {
  ...
  if (strcmp(req->method, "HEAD") != 0) {
    ...
    if (fd <0)
      log_exit("faled to open %s: %s", info->path, strerror(errno));
    for(;;) {
...
static void not_found(struct HTTPRequest *req, FILE *out) {
  ...
  fprintf(out, "Fontent-Type: text/html\r\n");
  fprintf(out, "\r\n");
  if(strcmp(req->method, "HEAD") != 0) {
    fprintf(out, "<html>\r\n");
...
static long content_length(struct HTTPRequest *req) {
  ...
  if (!val) return 0;
  len = atol(val);
  if (len < 0) log_exit("negative Content-Length");
  return len;
}
